...
def make_unit(dir_index):
    n=0
    m=1
    for i in range(100,len(nene_table),100):
        nene_table[n:i].to_csv((dir_name + backslash + nene_dir + str(dir_index) + backslash + nene_file +str(m)+ csv), encoding="949", mode='w',index=True)
        m += 1
        n = int(i)
    nene_table[i:].to_csv((dir_name + backslash + nene_dir + str(dir_index) + backslash + nene_file +str(m)+ csv), encoding="949", mode='w',index=True)
