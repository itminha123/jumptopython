...
def student_lecture_search(search):
    ...
    for i in result:
        if "현재 수강정보" in i["수강정보"]:
            for j in i["수강정보"]["현재 수강정보"]:
                if student_search_input in j[search]:
                    search_list.append(i)
    search_count(search_list)
...
while True:
    ...
    if student_inupt == '1':
        with open('ITT_student.text', 'r') as file:
            ...
            while True:
                student_lecture=input("1. 새로운 강의 추가 2. 종료")
        ...
        for i in result:
            if delete_id == i["ID"]:
                ...
                if delete == "0":
                    break
                if delete == '1':
                    delete_again = input("정말 삭제 하시겠습니까? ( y or n ) \n입력: ")
                    if delete_again == ('y' or 'Y'):
                        i.clear()
                        result.remove({})
                        json_write()
                    elif delete_again == ('n' or 'N'):
                        break
                elif delete == '2':
                    if "현재 수강정보" in i["수강정보"]:
